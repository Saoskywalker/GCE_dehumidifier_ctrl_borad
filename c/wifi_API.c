...
void WIFI_data_update(void)
{
     ...
     case DPID_FAN_SPEED_ENUM: //风速(可下发可上报)
     {
          ...
          wifi_tx_status = mcu_dp_value_update(DPID_FAN_SPEED_ENUM, Wifi_Updata.SYS_Fan_Tyde);
     }
     break;
     case DPID_MODE: //模式(可下发可上报)
     {
          Wifi_Updata.SYS_Mode = G_SYS_Mode;
          wifi_tx_status = mcu_dp_enum_update(DPID_MODE, Wifi_Updata.SYS_Mode);
     }
     break;
     case DPID_HUMIDITY_INDOOR: //室内湿度(只上报)
     {
          Wifi_Updata.Room_HUM = GET_ROOM_HUM();
          wifi_tx_status = mcu_dp_enum_update(DPID_HUMIDITY_INDOOR, Wifi_Updata.Room_HUM); //枚举型数据上报;
     }
     break;
     case DPID_FAULT:
     { //故障告警(只上报)
     ...
     case DPID_TANK_FULL: //水箱水位(只上报)
     {
          ...
          wifi_tx_status = mcu_dp_value_update(DPID_TANK_FULL, Wifi_Updata.Water_Full); // VALUE型数据上报;
     }
     break;
     case DPID_DEFROST_STATUS: //除霜状态(只上报)
     {
          Wifi_Updata.SYS_defrost_status = G_Def_Reg.Defrost_status;
          wifi_tx_status = mcu_dp_value_update(DPID_DEFROST_STATUS, Wifi_Updata.SYS_defrost_status); // VALUE型数据上报;
     }
     break;
     case DPID_PUMP: //自动排水(可下发可上报)
     {
          Wifi_Updata.SYS_PUMP_status = G_Pump_Status;
          wifi_tx_status = mcu_dp_bool_update(DPID_PUMP, Wifi_Updata.SYS_PUMP_status); // BOOL型数据上报;
     }
     break;
     default:
     {
...
void data_update(void) // wifi模块上电 全部上报一次
{
     ...
     else if (Wifi_Updata.SYS_Fan_Tyde != G_SYS_Fan_Tyde)
     {
          mcu_dp_type = DPID_FAN_SPEED_ENUM;
     }
     else if (Wifi_Updata.SYS_Mode != G_SYS_Mode) //默认确认
     {
     ...
     else if (Wifi_Updata.SYS_PUMP_status != G_Pump_Status)
     {
...
void Wifi_UPdata_for_SYSdata_Deal(void)
{
     ...
     if (G_Sys_Err.hum_Sensor_err)
     {
          err_buf |= 0x04;
     }

     if (GET_COM_STATUS() == ERROR) //通信错误
     {
          err_buf |= 0x08;
     }

     if (G_Sys_Err.temp_comp_err)
     {
          err_buf |= 0x10;
     }
